module FavoritosHelper
  ...
  def helper_favo(bazar, empresa, nombre, pre)
    ...
    if (!fav.nil?)
      "<div class='#{pre}-favo#{bazar}-#{empresa}'><a href='#' title='Quitar de Favoritos' onclick='$(\".#{pre}-favo#{bazar}-#{empresa}\").load(\"/favorito/delfav?bazar=#{bazar}&empresa=#{empresa}&pre=#{pre}\")'><img src='/images/favorito.png'></a></div>".html_safe
    else 
      "<div class='#{pre}-favo#{bazar}-#{empresa}'><a href='#' title='Agregar a Favoritos' onclick='$(\".#{pre}-favo#{bazar}-#{empresa}\").load(\"/favorito/addfav?bazar=#{bazar}&empresa=#{empresa}&nombre_empresa=#{nombre.gsub(' ','_')}&pre=#{pre}\")'><img src='/images/addfav.png'></a></div>".html_safe
    end
  end 
