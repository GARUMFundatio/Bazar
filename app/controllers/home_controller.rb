class HomeController < ApplicationController
  ...
  def index
    ...
    @ofertas = Bazarcms::Oferta.where("tipo = 'O' and empresa_id <> ? and date(fecha_hasta) >= date(?)", current_user.id, Time.now).order("fecha desc")
    @demandas = Bazarcms::Oferta.where("tipo = 'D' and empresa_id <> ? and date(fecha_hasta) >= date(?)", current_user.id, Time.now).order("fecha desc")
  ...
  def rating 
    ...
    if (params[:bazar].to_i == BZ_param("BazarId").to_i && params[:id].to_i == current_user.id )
      ...
      rating = Bazarcms::Rating.where("( ori_bazar_id = ? and ori_empresa_id = ? and des_bazar_id = ? and des_empresa_id = ? )", 
                                      BZ_param("BazarId").to_i, current_user.id, params[:bazar].to_i, params[:id].to_i).limit(1)
