   ...
   puts @paises.inspect
    ...
    respond_to do |format|
      format.json {
         @info = []
         for pais in @paises
           @info << {:label => "#{pais.descripcion} ( ~ #{pais.total_empresas_mercado} empresas)", :value => "#{pais.descripcion}", :id => "#{pais.id}", :ayuda => "Capital: #{pais.capital} Codigo ISO: #{pais.codigo}", :total => "#{pais.total_empresas_mercado}"}
         end
         render :json =>  @info  }
    end
