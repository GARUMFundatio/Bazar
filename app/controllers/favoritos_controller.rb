class FavoritosController < ApplicationController
  ...
  def addfav
    ...
    Actividad.graba("Ha añadido a favoritos la empresa: <a href='#{Cluster.find_by_id(BZ_param('BazarId')).url}/bazarcms/empresas/#{current_user.id}?bazar_id=#{BZ_param('BazarId')}'>#{params[:nombre_empresa].gsub('_',' ')}</a>.", "USER",  BZ_param("BazarId"), current_user.id, nombre)
